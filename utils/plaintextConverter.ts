import type {
  MindElixirData,
  NodeObj
} from "~node_modules/mind-elixir/dist/types"
import type { Arrow } from "~node_modules/mind-elixir/dist/types/arrow"
...
export function plaintextToMindElixir(plaintext: string): MindElixirData {
  ...
  const arrows = context.arrowLines
    .map(({ content }) => parseArrow(content, context))
    .filter((a): a is Arrow => a !== null)

  const summaries = context.summaryLines
    .map(({ content, parentChildren, parentId }) =>
      parseSummary(content, parentChildren, parentId)
    )
