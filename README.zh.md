...
> 以下内容可直接复制到 Chrome Web Store / Firefox Add-ons / Edge Add-ons 的扩展描述中。

一个 40 分钟的视频，你需要的可能只是 3 个重点；一篇万字长文，和你相关的也许只有一段话。

M10C 是一款浏览器扩展，能把 YouTube、Bilibili 视频和网页文章一键变成 AI 总结和思维导图——先看重点，再决定要不要花时间看完。

**✨ 你能用它做什么**

- 🎬 **视频：先看总结再决定看不看** —— 自动读取视频字幕，一键生成概要与关键要点；点任意要点直接跳到视频对应位置，不用再拖着进度条找
- 📄 **文章：长文一屏读完** —— 自动提取网页正文，主要观点、关键信息、话题标签直接摆在面前
- 🧠 **导图：看完的东西留得下来** —— 总结自动变成结构化思维导图，支持交互浏览、编辑和导出，复习、分享都方便
- 🆓 **已生成的内容免费拿** —— 通过内置托管模型（星星算力）生成过的内容会存到云端；再遇到已生成过的视频或文章，直接免费领取现成结果，不用再花一次生成的钱（每人每月 5 次）
- 🔒 **API Key 不离开你的浏览器** —— 支持 Google Gemini（免费）、OpenAI、Claude 等自带 Key；不想配置？登录 Mind Elixir 账号用内置托管模型，开箱即用

**怎么开始：** 安装后打开任意视频或文章，点一下"生成思维导图"，剩下的交给 M10C。
...
- 💾 **智能缓存，重复内容免费领取**: 内置托管模型生成过的内容会存到云端，再遇到同一视频或文章，可直接免费领取现成结果，不用重复生成（每人每月 5 次）
