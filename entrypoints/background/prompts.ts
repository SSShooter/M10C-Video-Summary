...
UNIFIED PREFIX RULE (applies to EVERY line without exception):
Every line MUST start with "- " (dash + space). The special markers "}", "}:n" and ">" come AFTER the "- " prefix. Each line uses EXACTLY ONE marker — NEVER merge or chain markers together.

Format Definition (note that EVERY line starts with "- "):
- Core Topic Title
  - Main Point A
    - Detail A-1
    - Detail A-2
...
3. Every line MUST start with "- ". After the "- " prefix, a line must be exactly ONE of the following:
   - plain node text (optionally followed by [^id] and/or a style JSON)
   - "} Summary Text" — one space after "}"
   - "}:n Summary Text" — one space after "}:n"
   - "> [^id1] <-Label-> [^id2]" — a relation line (see rule 7)
   Combining markers (such as "}>") is STRICTLY FORBIDDEN.
...
6. Summary nodes (must be a standalone line at the same indentation as the siblings they summarize):
   - "- } Summary Text" summarizes ALL previous siblings at the same level.
   - "- }:n Summary Text" summarizes the previous n siblings at the same level.
   - There MUST be exactly one space between "}" (or "}:n") and the summary text.
7. Relationship links (must be a standalone line, may appear at any position in the outline):
   - Bidirectional: "- > [^id1] <-Label-> [^id2]"
   - Unidirectional: "- > [^id1] >-Label-> [^id2]"
   - Both [^id] values MUST reference IDs defined earlier via rule 4. A relation line MUST NOT contain "}".
...
Common mistakes — NEVER output any of these:
- "> [^id1] <-Label-> [^id2]"   WRONG: missing "- " prefix
- "- }> [^id1] <-Label-> [^id2]"  WRONG: "}" must never be combined with ">"
- "- }Summary Text"             WRONG: missing space after "}"
- "- } > [^id1] <-Label-> [^id2]" WRONG: relation and summary must be separate lines
`
  },
